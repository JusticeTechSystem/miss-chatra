...
function getVersion() {
  try {
    ...
    // Priority: tag > version+codename > version alone
    if (v && v.tag && typeof v.tag === "string" && v.tag.length > 2) return v.tag;
    if (v && v.version && typeof v.version === "string") {
      const code = (v.codename && typeof v.codename === "string") ? `_${v.codename}` : "_JT";
...
async function buildCtx({ sock, prefix, isOwner, isDev: dev }) {
  const pfx  = prefix || "/";
  ...
  // Measure real WA response speed via a lightweight async timing probe
  // We time how long it takes to prepare a sendMessage call (includes event loop lag)
  const _speedStart = Date.now();
  await new Promise(resolve => setImmediate(resolve));  // flush any pending I/O
  const speedMs = Date.now() - _speedStart;
  // Also measure Node event-loop lag with high-res timer
  const _hrStart = process.hrtime.bigint();
  await new Promise(resolve => setTimeout(resolve, 0));
  const evLoopMs = Number(process.hrtime.bigint() - _hrStart) / 1e6;
  const finalSpeedMs = Math.max(speedMs, evLoopMs).toFixed(2);
  // CPU usage %
  let cpuPct = 0;
  try {
    const cpus = os.cpus();
  ...
  const ver = VERSION();
  return { pfx, dev, uptime, upsh, pct, bar, name, mode, sorted, total_cmds, botName, total_plugins, speedMs: finalSpeedMs, usageLine, ver };
}
...
{ id:2, name:"Neon Terminal", emoji:"⚡", smart:false,
  render(c) {
    const {pfx,uptime,name,mode,pct,bar,sorted,ver} = c, B = [];
...
{ id:3, name:"Luxury Card", emoji:"🏆", smart:false,
  render(c) {
    const {pfx,upsh,name,mode,pct,bar,sorted,ver} = c;
    const L = "─────────────────────────────", B = [];
...
{ id:4, name:"Military Ops", emoji:"🛡️", smart:false,
  render(c) {
    const {pfx,uptime,name,mode,pct,bar,sorted,ver} = c;
    const TK = "▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓", TN = "▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬", B = [];
...
{ id:9, name:"Dark Void", emoji:"🖤", smart:false,
  render(c) {
    const {pfx,uptime,name,pct,bar,sorted,ver} = c;
    const V = "▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪▪", B = [];
...
{ id:11, name:"Classic Dashboard", emoji:"📂", smart:true,
  index(c) {
    const {name,uptime,pfx,sorted,ver} = c;
...
{ id:15, name:"Galaxy Dashboard", emoji:"🌌", smart:true,
  index(c) {
    const {name,upsh,pfx,sorted,ver} = c, S = "✦ ✧ ✦ ✧ ✦ ✧ ✦ ✧ ✦ ✧ ✦ ✧ ✦";
...
module.exports = {
  ...
  async run({ sock, m, args, command, chatId, userId, reply, prefix, isOwner, isDev }) {
    const pfx  = prefix || "/";
    const ver  = VERSION();  // needed by ping, runtime, and all menu themes
    ...
    const ctx   = await buildCtx({ sock, prefix: pfx, isOwner, isDev });
