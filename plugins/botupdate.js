...
async function downloadAndExtract(zipUrl, tmpDir) {
  ...
  const _isObfuscated = (root) => {
    try {
      ...
      const head = require("fs").readFileSync(p, "utf8").slice(0, 800);
      return head.includes("pbkdf2Sync") || head.includes("Obfuscationary") ||
             head.includes("_ITS") || (head.includes("(async()=>") && head.includes("require('crypto')"));
    } catch { return false; }
  };
