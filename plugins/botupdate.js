...
async function downloadAndExtract(zipUrl, tmpDir) {
  ...
  // Obfuscation guard — refuse to apply any update where index.js is encrypted
  const _isObfuscated = (root) => {
    try {
      const p = require("path").join(root, "index.js");
      if (!require("fs").existsSync(p)) return false;
      // Read first 2KB — enough to cover the Obfuscationary wrapper header
      const head = require("fs").readFileSync(p, "utf8").slice(0, 2048);
      // Match ONLY the exact Obfuscationary runtime signatures — all are very specific
      // to avoid false positives on plain source code that may contain common strings.
      const signatures = [
  ...
  if (_isObfuscated(srcRoot)) {
    throw new Error(
      "Update aborted: the downloaded index.js is obfuscated (Obfuscationary detected).\n" +
      "Push the plain, unobfuscated source to GitHub before running /update."
    );
  }
