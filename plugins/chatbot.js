...
module.exports = {
  ...
  handleMessage: async ({ sock, m, chatId, userId, text, isGroupChat, sender }) => {
    ...
    if (isGroupChat) {
      ...
      // In DMs: only active if chatbot_dm_enabled is set
      if (!state.chatbot_dm_enabled) return false;
    }
    ...
    const isGreeting = GREETINGS.some(g => {
      // Exact match
      if (msgLower === g) return true;
      // Starts with it followed by space/punctuation
      if (msgLower.startsWith(g + " ") || msgLower.startsWith(g + ",") ||
          msgLower.startsWith(g + "!") || msgLower.startsWith(g + "?")) return true;
      // Word boundary match anywhere in message (for "miss chatra" etc)
      if (g.length > 4) {
  ...
  run: async ({ sock, m, args, command, chatId, userId, reply, isOwner, isDev, isAdmin, prefix }) => {
    ...
    if (command === "chatbot") {
      ...
      const on = isGroup(chatId) ? !!gsGet(chatId).chatbot_on : !!state.chatbot_dm_enabled;
